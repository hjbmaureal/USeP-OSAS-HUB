 ...
 if (isset($_POST['currPass']) && isset($_POST['newPass'])&& isset($_POST['confirmNewPass'])) {
    ...
    //$query
      if(!file_exists($_FILES['image']['tmp_name'])){
        $file = "";
        $query = "UPDATE alumni SET contact='$contact', email_add = '$email'  WHERE id='$alumni';";
      } else {
          $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
          $query = "UPDATE alumni SET profile_pic = '$file',contact='$contact', email_add = '$email' WHERE id='$alumni';";
      }
      ...
      if(mysqli_query($conn, $query))
     {
       ...
       swal({
          ...
          type: "warning"
            ...
            setTimeout(function () {
              window.location.href="../users/alumni/user-profiles.php";
              }, 500);
              });
              </script>'; 
