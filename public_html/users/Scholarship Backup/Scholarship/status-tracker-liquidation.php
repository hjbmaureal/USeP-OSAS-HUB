<?php
  ...
  if(isset($_POST['update-liquidation'])){
    if(isset($_POST['select'])){
      foreach($_POST['select'] as $id){
        $statusLiquidation = 'DONE';
        mysqli_query($conn,"UPDATE grantee_history SET status_liquidation = '$statusLiquidation', liquidation_set_date = current_date() WHERE id = '$id'");
      }
      header("Location: status-tracker-liquidation.php?operation=success&set-status");
    }else{
      header("Location: status-tracker-liquidation.php?selection=empty");
    }
  }
  if(isset($_POST['unset-liquidation'])){
    if(isset($_POST['select'])){
      foreach($_POST['select'] as $id){
        $statusLiquidation = 'NOT DONE';
        mysqli_query($conn,"UPDATE grantee_history SET status_liquidation = '$statusLiquidation', liquidation_set_date = NULL WHERE id = '$id'");
      }
      header("Location: status-tracker-liquidation.php?operation=success&unset-status");
    }else{
      header("Location: status-tracker-liquidation.php?selection=empty");
    }
  }
...
<html lang="en">
  ...
  <body class="app sidebar-mini rtl" onload="initClock()">
      ...
      <ul class="app-menu font-sec">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li><a class="treeview-item active" href="status-tracker-liquidation.php">Liquidation</a></li>
    ...
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <!-- MAIN CONTENT -->
      <div class="main-content-billing">
              ...
              <div class="tile">
                <div class="tile-body">
                  ...
                  <div class="float-left">
                    <h4>Status Tracker - Liquidation</h4>
                  </div>
                  <div class="float-right">
                    <p>
                      <input type="button" value="Export" id="export-button" class="btn btn-primary icon-btn">
                      <input type="submit" name="update-liquidation" value="Set" class="btn btn-warning icon-btn">
                      <input type="submit" name="unset-liquidation" value="Unset" class="btn btn-danger icon-btn">
                    </p>
                  </div>
                  ...
                  <div id="liquidation-table-container">
                    <table class="table table-hover table-bordered" id="liquidation-table">
                      <thead>
                        <tr>
                          <th><input type="checkbox" id="select-all-liquidation"></th>
                          <th>Surname</th>
                          <th> First Name</th>
                          <th>Middle Name</th>
                      ...
                      <tbody id="liquidation-table-body">
                        <?php
                          ...
                          if(isset($_POST['sortScholarshipProgram']) && isset($_POST['sortCourse']) && isset($_POST['status']) && isset($_POST['sortSemester']) && isset($_POST['semesterToYear']) && isset($_POST['semesterFromYear']) && isset($_POST['record-status'])){
                            ...
                            if(!empty($sortStatus)){
                              $query .= " AND sgi.liquidation_status = '$sortStatus'";
                            }
                            if($sortCourse != 0){
                          ...
                          if($result = mysqli_query($conn,$query)){
                            while($row = mysqli_fetch_array($result)){
                              ...
                              echo '
                                  ...
                                  <td>'.$row['liquidation_status'].'</td>
                                  <td>'.$row['liquidation_set_date'].'</td>
                                </tr>
                              ';
                            }
                          }
      ...
      if(isset($_GET['operation']) && isset($_GET['set-status']) && $_GET['operation'] == 'success'){
        echo '
          <script>
            Swal.fire(
              "Update successful!",
              "Row/s has been updated to DONE.",
              "success"
            )
          </script>
        ';
      ...
      if(isset($_GET['operation']) && isset($_GET['unset-status']) && $_GET['operation'] == 'success'){
        echo '
          <script>
            Swal.fire(
              "Update successful!",
              "Row/s has been reverted to NOT DONE.",
              "info"
            )
          </script>
        ';
    ...
    <script type="text/javascript">
      ...
      if(typeof window.history.pushState == 'function') {
        window.history.pushState({}, "Hide", "status-tracker-liquidation.php");
      }
      //PREVENT RESEND
      ...
      $(document).ready(function(){
        ...
        //Data Tables
        var liquidation_table = $('#liquidation-table').DataTable({
          "ordering": false
        });

        //SELECT ALL CHECKBOX
        $("#liquidation-table-container #select-all-liquidation").click(function(){
          $("#liquidation-table-container input[type='checkbox']").prop('checked', this.checked);
        })

        //EXPORT FUNCTION
        $("#export-button").click(function(){
          liquidation_table.column(0).visible(false);
          $("#liquidation-table").table2excel({
              // exclude CSS class
              exclude: ".ms-excel",
              name: "Worksheet Name",
          filename: "LiquidationStatus.xls" //do not include extension
          });
          setTimeout(() => {
            liquidation_table.column(0).visible(true);
          }, 3000);
        });
