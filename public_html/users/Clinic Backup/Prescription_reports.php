          ...
          <div class="col-md-12">
            <div class="tile">
              <div class="tile-body">
                <div>
                <div>
                <div class="float-left"><h4>Prescription Reports</h4></div>
                  </div>
                  <br><br>
                  <div class="row">
                    <div class="col-auto">
		...
		<thead>
			...
			<th><h4><center> Prescription Reports</center></h4></th>
			 ...
			 <table class="table table-hover table-bordered reports-list" id="myTable">
                  ...
                  <tbody> 
				  <?php
				  $sql = "SELECT consultation_type.consultation_type,consultation.communication_mode_first_option,consultation.patient_id, consultation.prescription_date_issued,consultation.prescription_details,CONCAT(student.first_name,' ',student.middle_name,' ', student.last_name) as name, course.title from consultation join student on consultation.patient_id=student.Student_id join consultation_type on consultation.consultation_type=consultation_type.type_id join course on student.course_id=course.course_id where consultation.prescription_details!='NULL' order by prescription_date_issued ASC";

                    $res = $db->query($sql);
                    $cnt=1;
                    while($row = $res->fetch_assoc()) {
					$date =date_create($row['prescription_date_issued']);
