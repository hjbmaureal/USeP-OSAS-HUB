...
if(isset($_POST['Submit'])){ 
  ...
  $school_year=$_POST['schoolyear'];
   $semester=$_POST['semester'];
    ...
    $sqlname = mysqli_query($db, "SELECT patient_id, CONCAT(first_name,' ',middle_name,' ',last_name) as fullname , type from patient_list where patient_id = '$patient_id'");
    ...
    if(empty($patient_id)||empty($type)||empty($mode)||empty($problem)) { 
      ...
      $message = 'added a new complaint';
        $result ="INSERT INTO consultation(patient_id,consultation_type,staff_id,communication_mode,problems,date_filed, status,semester, school_year) VALUES('$patient_id','$type','$staff_id','$mode','$problem',now(),'$status','$semester', '$school_year')";
        ...
        //char2x rani hahahahha
          ...
          echo "<script type='text/javascript'> document.location = 'StudentConsultationHistory.php'; </script>";
                exit();
    }else{
