...
<form method="post">   
                         ...
                         <h6 class="font-weight-bold">Tooth: </h6> 
                            <div class="form-row">
                                      <div class="form-group col-md-12">
                                          <textarea class="form-control rounded-10" required="required" name="tooth" id="tooth" rows="4" placeholder="Type here..."></textarea>
                                      </div>
                          </div>
                          <h6 class="font-weight-bold">Surface: </h6> 
                            <div class="form-row">
                                      <div class="form-group col-md-12">
                                          <textarea class="form-control rounded-10" required="required" name="surface" id="surface" rows="4" placeholder="Type here..."></textarea>
                                      </div>
                          </div>
                        ...
                        <br><br>
                               ...
                               <div class=""> 
                                          ...
                                          <b>Select Medication:</b>
                                            <select  name="art_fotoselectsss" id="art_fotoselectsss" class="form-control" style="height: 35px;width: 250px" data-table="reports-list">
                                      ...
                                      <h6 class="font-weight-bold">Quantity: </h6>  
                                          <input  type="number" name="quan1" id="quan1" class="form-control" ></input>
                                           <div>
                                            <button type="button" name="addd" id="addd" class="btn btn-success">Add</button></div>
                                      ...
                                      <div class="form-row">
                                      <div class="form-group col-md-12">
                                          <textarea class="form-control rounded-10" name="art_testo" id="i255x" rows="4" placeholder="If no prescription provided just put N/A..."></textarea>
                                     ...
                                     &ensp;&ensp;
                                            <select  name="doctor" id="doctor" class="form-control" style="height: 35px;width: 250px" data-table="reports-list">
                                                 ...
                                                 $Doctor="Doctor";
                                                  // Feching active mode of communication
                                                  $sql=mysqli_query($db,"select CONCAT(staff.first_name,' ',staff.middle_name,' ',staff.last_name) as name from staff where staff.position='$Doctor'");
                                                  while($result=mysqli_fetch_array($sql))
                                                  {    
                                                  ?>
                                                  <option class="select-item" value="<?php echo htmlentities($result['name']);?>"><?php echo htmlentities($result['name']);?></option>
                                                  <?php }
...
<?php
  ...
  if(isset($_POST['Submit'])){   
    ...
    // checker
    if(empty($tooth)||empty($surface)||empty($diagnosis)|| empty($treatment)|| empty($remarks)) { 
        ...
        $result ="UPDATE consultation 
                  SET tooth='$tooth', surface='$surface', diagnosis='$diagnosis',status='$status', treatment='$treatment', remarks='$remarks',prescription_details='$pressum',dose_interval='$guide',recommended_doctor='$doctor',prescription_date_issued=now()
                  WHERE id ='$id'";
...
Art_AddFotorz = (e) => {
 var b = "Quantity = ";
 document.querySelector('#i255x').value += document.querySelector('#art_fotoselectsss').value+"\r\n";
 document.querySelector('#i255x').value += b+document.querySelector('#quan1').value+"\r\n";
}
