...
if (isset($_POST['pass'])){
  ...
  if ($user && password_verify($pass, $user['password'])){
        ...
        $tab = mysqli_query($conn,"SELECT * from org_filess where ID='$User_id'");
        $res = mysqli_fetch_array($tab);

                if($res){
                  $name = $res['Org'];
                      $submittedby= $res['Org_pres_gov'];
                      
                                                    $query1 = "UPDATE org_filess set status =1 Where ID ='$User_id' ";
                                                 $run1 = mysqli_query($conn, $query1);

                                                 echo '<script>
                                                    swal({
                                                    title: "'.$name.' : Approved files",
                                                  
                                                    type: "success"
                                                    }, function () {
                                                    setTimeout(function () {
                                                    window.location.href="../UnrecognizedOrg.php";
                                                    }, 500);
                                                    });
                                                    </script>'; 
