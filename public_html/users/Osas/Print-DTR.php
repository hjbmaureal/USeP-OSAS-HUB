<?php 
  ...
  $query=mysqli_query($conn,"SELECT applicant_id, DATE_FORMAT(time_in, '%m') AS month, DATE_FORMAT(time_in, '%d') AS day_of_month, DATE(time_in) AS day, (SELECT DATE_FORMAT(time_in, '%h:%i %p') FROM sl_dtr WHERE applicant_id = '".$app_id."' and DATE(time_in) = day ORDER BY time_in LIMIT 1) as morning_timein, (SELECT DATE_FORMAT(time_out, '%h:%i %p') FROM sl_dtr WHERE applicant_id = '".$app_id."' and DATE(time_in) = day and date_format(time_in,'%p') = 'AM' order by time_out DESC LIMIT 1) as morning_timeout, (SELECT DATE_FORMAT(time_in, '%h:%i %p') FROM sl_dtr WHERE applicant_id = '".$app_id."' and DATE(time_in) = day and date_format(time_in,'%p') = 'PM' ORDER BY time_in LIMIT 1) as afternoon_timein, (SELECT DATE_FORMAT(time_out, '%h:%i %p') FROM sl_dtr WHERE applicant_id = '".$app_id."' and DATE(time_in) = day ORDER BY time_out DESC LIMIT 1) as afternoon_timeout, TIME_FORMAT(SEC_TO_TIME(SUM(TIME_TO_SEC(diff))), '%H:%i') AS hours_worked, sem_year FROM sl_dtr GROUP BY day HAVING applicant_id = '".$app_id."' AND day BETWEEN '".$from."' AND '".$to."' ORDER BY day;");
