        ...
        <ul class="app-menu font-sec">
          ...
          <li class="treeview"><a class="app-menu__item active" href="#" data-toggle="treeview"><i class="app-menu__icon far fa-registered"></i><span class="app-menu__label">Account Records</span><i class="treeview-indicator fa fa-angle-right"></i></a>
            <ul class="treeview-menu">
              ...
              <li><a class="treeview-item active" href="StudentAccounts.php">Student Accounts</a></li>
          ...
          <div class="col-md-12">
            <div class="tile">
              <div class="tile-body">
                <div>
                <div>
                <div class="float-left"><h4>Student Accounts</h4></div>
                  </div>
                  <br><br>
                  <div class="row">
                    <div class="col mt-1">
<form method="POST" action="<?php echo $_SERVER['PHP_SELF']?>">
                  ...
                  <div class="inline-block">
                    Year Level
                    <BR>
                    <select class="bootstrap-select size-150px" name="year_level_dropdwn" id="year_level_dropdwn">
                      <option class="select-item" value="0">All</option>
                        <?php
                                    
                                   
                                    $sql="SELECT DISTINCT year_level FROM studentdetails";
                                    $result = mysqli_query($conn,$sql);
                                    while($row=mysqli_fetch_array($result)){
                                          echo "<option class='select-item'";
                                          if(isset($_POST['year_level_dropdwn']) && $_POST['year_level_dropdwn'] == $row['year_level']){
                                              echo "selected value='".$row['year_level']."'>".$row['year_level']."</option>";
                                          }else{
                                            echo "value='".$row['year_level']."'>".$row['year_level']."</option>";
                                          }
                                    }
                                ?>
                            </select>
...
<div class="inline-block float mr-7 mt-3">
                ...
                <div class="table-responsive">
                  ...
                  <?php
                  include("conn.php");
                  $sql = "SELECT * FROM studentdetails where student_id is not null";
                  if (isset($_POST['course_dropdwn']) || isset($_POST['year_level_dropdwn'])) {
                         # code...
                        

                      
                        $course_comp = $_POST['course_dropdwn'];
                        $year_level_comp = $_POST['year_level_dropdwn'];

                        

                        if ($course_comp != "0") {
                          # code...
                          $sql .= " AND coursetitle like '$course_comp%' ";
                        }
                        if ($year_level_comp != "0") {
                          # code...
                          $sql .= " AND year_level like '$year_level_comp%' ";
                        }
        ...
        while($res = mysqli_fetch_array($tab)) {         
            ...
            <td><?php echo $res['account_status']; ?></td>
                     ...
                     <td>
                      <button class="btn btn-info btn-sm" data-toggle="modal" data-target="#exampleModalLong<?php echo $res['student_id']; ?>"><i class="fas fa-eye"></i></button>
                      <button a href="#exampleModal<?php echo $res['student_id']; ?>" role="button" class="btn btn-warning btn-sm" data-toggle="modal"><i class="fas fa-edit"></i></button>

                      <?php include('include/Student-view.php'); ?>

                                            
                   <?php 
                    if($res['account_status'] == "Active"){ ?>
                            
                        <button data-target="#disable-dialog" role="button" class="btn btn-success btn-sm" disabled="disabled">Enable</button>

                        <button data-target="#disable-dialog<?php echo $res['student_id']; ?>" role="button" class="btn btn-danger btn-sm" data-toggle="modal" id="formid"  onclick="toggleClasses()">Disable</button>
                   ...
                   ?>
                          ...
                          <button data-target="#enable-dialog<?php echo $res['student_id']; ?>" role="button" class="btn btn-success btn-sm" data-toggle="modal" id="formid" onclick="toggleClasses()" >Enable</button>

                          <button data-target="#disable-dialog" disabled="disabled" class="btn btn-danger btn-sm">Disable</button>
 
                      <?php
                    }
                     include('include/Student-edit.php');
                     include('include/Student-disable.php');
                     include('include/Student-enable.php'); ?>
    ...
    $(this).closest('table').find('td input:checkbox').prop('checked', this.checked);
      ...
      <script type="text/javascript">
        $(document).ready(function(){
                    $(document).on('click', '[data-role="prodbtn"]', function(){
                      var stats = $(this).data('status');
                      var id=$(this).data('id');
                      var first=$(this).data('fllname');
                      var message=$(this).data('message');
                     

                      $('#disable-dialog').modal('show');
                      $('#eid').val(id);
                      $('#efirst').val(first);
                      $('#stats').val(stats);
