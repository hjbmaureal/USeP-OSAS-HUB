<?php
  ...
  if(isset($_POST['update-payroll'])){
    if(isset($_POST['select'])){
      foreach($_POST['select'] as $id){
        $statusPayroll = 'DONE';
        mysqli_query($conn,"UPDATE grantee_history SET status_payroll = '$statusPayroll', payroll_set_date = current_date() WHERE id = '$id'");
      }
      header("Location: status-tracker-payroll.php?operation=success&set-status");
    }else{
      header("Location: status-tracker-payroll.php?selection=empty");
    }
  }
  if(isset($_POST['unset-payroll'])){
    if(isset($_POST['select'])){
      foreach($_POST['select'] as $id){
        $statusPayroll = 'NOT DONE';
        mysqli_query($conn,"UPDATE grantee_history SET status_payroll = '$statusPayroll', payroll_set_date = NULL WHERE id = '$id'");
      }
      header("Location: status-tracker-payroll.php?operation=success&unset-status");
    }else{
      header("Location: status-tracker-payroll.php?selection=empty");
    }
  }
  ...
  <body class="app sidebar-mini rtl" onload="initClock()">
      ...
      <ul class="app-menu font-sec">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li><a class="treeview-item active" href="status-tracker-payroll.php">Payroll</a></li>
              ...
              <div class="tile">
                <div class="tile-body">
                  ...
                  <div class="float-left">
                    <h4>Status Tracker - Payroll</h4>
                  </div>
                  <div class="float-right">
                    <p>
                      <input type="button" value="Export" id="export-button" class="btn btn-primary icon-btn">
                      <input type="submit" name="update-payroll" value="Set" class="btn btn-warning icon-btn">
                      <input type="submit" name="unset-payroll" value="Unset" class="btn btn-danger icon-btn">
                    </p>
                  </div>
                  ...
                  <div id="payroll-table-container">
                    <table class="table table-hover table-bordered" id="payroll-table">
                      <thead>
                        <tr>
                          <th><input type="checkbox" id="select-all-payroll"></th>
                          <th>Surname</th>
                          <th> First Name</th>
                          <th>Middle Name</th>
                      ...
                      <tbody id="payroll-table-body">
                        <?php
                          ...
                          if(isset($_POST['sortScholarshipProgram']) && isset($_POST['sortCourse']) && isset($_POST['status']) && isset($_POST['sortSemester']) && isset($_POST['semesterToYear']) && isset($_POST['semesterFromYear'])){
                            $semester_year = null;
                            $year = null;
                            ...
                            if(!empty($sortStatus)){
                              $query .= " AND sgi.payroll_status = '$sortStatus'";
                            }
                            if($sortCourse != 0){
    ...
    <script type="text/javascript">
      ...
      if(typeof window.history.pushState == 'function') {
        window.history.pushState({}, "Hide", "status-tracker-payroll.php");
      }
      //PREVENT RESEND
      ...
      $(document).ready(function(){
        ...
        //SELECT ALL CHECKBOX
        $("#payroll-table-container #select-all-payroll").click(function(){
          $("#payroll-table-container input[type='checkbox']").prop('checked', this.checked);
        })

        //EXPORT FUNCTION
        $("#export-button").click(function(){
          payroll_table.column(0).visible(false);
          $("#payroll-table").table2excel({
              // exclude CSS class
              exclude: ".ms-excel",
              name: "Worksheet Name",
          filename: "Payroll.xls" //do not include extension
          });
          setTimeout(() => {
            payroll_table.column(0).visible(true);
          }, 3000);
        });
