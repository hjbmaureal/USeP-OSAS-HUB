...
if($request['usertype']=='Student'){
    $result=mysqli_query($conn,"insert into `notif` (user_id, message_body, time, link, message_status,office_id) values ('$patient_id', 'Admin" .' '. "".$message."',now(),'../users/Student/RequestMedCert.php', 'Delivered','3')");
    header("Location: admin-request.php");
}
if($request['usertype']=='Faculty Head'){
    $result=mysqli_query($conn,"insert into `notif` (user_id, message_body, time, link, message_status,office_id) values ('$patient_id', 'Admin" .' '. "".$message."',now(),'../users/Faculty/RequestMedCert.php', 'Delivered','3')");
    header("Location: admin-request.php");
}
