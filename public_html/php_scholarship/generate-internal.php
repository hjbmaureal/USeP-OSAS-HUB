  ...
  if(isset($_POST['generate-scholar'])){
    ...
    foreach($_POST['select'] AS $value){
      ...
      if($row = mysqli_fetch_array($result)){
        ...
        $card_location = ( $card_ver== 0) ? '../images/Scholarship_Cards/Internal/'.$row['Student_id'].'.png' : '../images/Scholarship_Cards/Internal/'.$row['Student_id'].'_'.$card_ver.'.png';
        ...
        if ($row['check_if_card_exists'] > 0 && file_exists($card_location)) {
          ...
          if (!strlen(trim($row['card_signed'])) > 0){
              ...
              $string = "Your scholarship card has been signed for the $notifSemester' ' $year ";
            ...
            $image1= '../images/scholarship_card_template/Internal.jpg';
            $image2='../images/scholarship_card_template/signature.png';
            $image1= imagecreatefromstring(file_get_contents($image1));
            $image2= imagecreatefromstring(file_get_contents($image2));
            imagecolortransparent($image2, imagecolorat($image2, 0, 0));
    ...
    header("Location: ../users/Scholarship/generate-internal-scholar-card.php?operation=success");
    } 
  }else{
  header("Location: ../users/Scholarship/generate-internal-scholar-card.php?operation=failed");
  }
header("Location: ../users/Scholarship/generate-internal-scholar-card.php?operation=unknown");
  ...
  function getCardLocation($studentid, $cardno){
    return $cardno == 0 ? '../images/Scholarship_Cards/Internal/'.$studentid.'.png' : '../images/Scholarship_Cards/Internal/'.$studentid.''.$cardno.'.png';
  }
